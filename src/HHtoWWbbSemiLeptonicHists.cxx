...
HHtoWWbbSemiLeptonicHists::HHtoWWbbSemiLeptonicHists(Context & ctx, const string & dirname): Hists(ctx, dirname){
  ...
  pt_jet2 = book<TH1F>("pt_jet1", "p_{T}^{jet 2}", 100, 10, 250);
  pt_jet3 = book<TH1F>("pt_jet1", "p_{T}^{jet 3}", 100, 10, 250);
  pt_jet4 = book<TH1F>("pt_jet1", "p_{T}^{jet 4}", 100, 10, 250);
  eta_jets = book<TH1F>("eta_jets", "#eta^{jets}", 40, -2.5, 2.5);
  eta_jets_rebin = book<TH1F>("eta_jets", "#eta^{jets}", 60, -5.0, 5.0);
